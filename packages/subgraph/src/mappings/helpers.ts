...
export const maticUsd = (): BigDecimal => {
  ...
  const maticUsd = quoter.try_quoteExactInputSingle(MATIC, USDC_MAINNET, 3000, TEN_18, BI_ZERO)
...
export const grtUsd = (): BigDecimal => {
  const quoter = UniswapQuoter.bind(QUOTER)
  const grtEth = quoter.try_quoteExactInputSingle(GRT, WETH9_ARBITRUM, 3000, TEN_18, BI_ZERO)
  if (grtEth.reverted) return BD_ZERO

  const ethUsdc = quoter.try_quoteExactInputSingle(WETH9_ARBITRUM, USDC_ARBITRUM, 500, TEN_18, BI_ZERO)
...
export const lptUsd = (): BigDecimal => {
  const quoter = UniswapQuoter.bind(QUOTER)
  const lptEth = quoter.try_quoteExactInputSingle(LPT, WETH9_ARBITRUM, 3000, TEN_18, BI_ZERO)
